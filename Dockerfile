...
RUN mkdir ~/docker_ws && \
    cd ~/docker_ws
RUN --mount=type=ssh git clone --recurse-submodules git@github.com:uga-robotics/AutonomousHexacopter.git src/AutonomousHexacopter
RUN cd src/AutonomousHexacopter/PX4-Autopilot && git checkout uga-dev && \
